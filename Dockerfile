...
FROM openjdk:${JDK_VERSION}-ea-slim AS jdk-build
...
RUN <<EOT
 ...
 $JAVA_HOME/bin/jlink \
  ...
  jcmd app AOT.end_recording
  curl -fsSL http://localhost/shutdown || echo "App Stopped!"
  sleep 2
...
# COPY --link --from=openjdk:${JDK_VERSION}-ea-slim $JAVA_HOME $JAVA_HOME
...
FROM openjdk:${JDK_VERSION}-ea-slim AS openjdk-hsdis
...
# FROM ghcr.io/graalvm/native-image-community:25-muslib AS graalvm-build
...
FROM openjdk:${JDK_VERSION}-ea-slim AS jshell
